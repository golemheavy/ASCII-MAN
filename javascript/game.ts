interface ITextElements {
	...
	directionsText: HTMLElement;
	winsText: HTMLElement;
	guessesLeftText: HTMLElement;
	lettersGuessedText: HTMLElement;
...
interface IGame {
	...
	currentWord: string;
	currentHint: string;
	revealedWordArray: string[];
	over: boolean;
	lastWordIndex: number;
	initializeValues: () => void;
	updateRevealedWordArray: (x: number, y: string) => void;
	updateDisplayedRevealedWord: () => void;
	setCurrentWord: (x: number) => void;
	newWordIndex:  (x: number) => number;
};
...
const game: IGame = {
	...
	setCurrentWord : function(x: number) {
...
const stickManIds: string[] = ["legs", "torso", "arms", "headg", "rope"];

function drawStickMan(x) {
	const targetDiv: HTMLElement = document.getElementById(stickManIds[x]);
	if (targetDiv) targetDiv.classList.remove("invisible");
}
